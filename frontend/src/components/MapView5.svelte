<script>
    ...
    try {
      ...
      navigator.geolocation.getCurrentPosition(
        (position) => {
          ...
          // Check if location is over a Medium/High crime tile
          checkCurrentLocationTile(userLocation.lat, userLocation.lng);
        },
        (error) => {
          locationPermissionStatus = 'denied';
  ...
  let currentTileSeverity = null;
  let currentTileData = null;
  let overlayMode = 'active'; // 'off', 'active', 'always'
  ...
  function cycleOverlayMode() {
    ...
    // Recheck current location to update overlay
    if (userLocation) {
      checkCurrentLocationTile(userLocation.lat, userLocation.lng);
    }
  }
  
  function checkCurrentLocationTile(lat, lng) {
    if (crimeData.length === 0) {
      // No data available
      if (overlayMode === 'always') {
        currentTileSeverity = 'NoData';
        currentTileData = null;
      } else {
        currentTileSeverity = null;
        currentTileData = null;
      }
      return;
    }
    
    // Find the tile containing current location
    let tileFound = false;
    for (const tile of crimeData) {
      if (lat >= tile.min_latitude && lat <= tile.max_latitude &&
        ...
        if (overlayMode === 'always') {
          // Always show overlay regardless of severity
          currentTileSeverity = severity;
          currentTileData = tile;
        } else if (overlayMode === 'active') {
          // Only show for Medium or High
    ...
    // No tile found - outside tiles or no data for this location
    if (!tileFound) {
      if (overlayMode === 'always') {
        currentTileSeverity = 'NoData';
        currentTileData = null;
  ...
  async function loadSimulationPath() {
    try {
      const response = await fetch('/data/path_simulation.json');
      const data = await response.json();
      
      if (data.routes && data.routes[0] && data.routes[0].geometry) {
        pathCoordinates = data.routes[0].geometry.coordinates;
        console.log(`Loaded ${pathCoordinates.length} path coordinates`);
      }
    } catch (error) {
      console.error('Failed to load simulation path:', error);
    }
  }
  ...
  function startWalkingSimulation() {
    ...
    // Calculate delay between points for 30 second total duration
    const totalDuration = 30000; // 30 seconds in milliseconds
    const intervalDelay = totalDuration / pathCoordinates.length;
    
    // Move to starting position
    const [startLng, startLat] = pathCoordinates[0];
    map.setView([startLat, startLng], 16);
    
    // Create or update marker and circle at starting position
    updateUserMarkerPosition(startLat, startLng);
    ...
    simulationInterval = setInterval(() => {
      ...
      const [lng, lat] = pathCoordinates[currentPathIndex];
  ...
  function pauseWalkingSimulation() {
    isPaused = true;
  ...
  function resumeWalkingSimulation() {
    if (pathCoordinates.length === 0 || currentPathIndex >= pathCoordinates.length) return;
  ...
  function updateUserMarkerPosition(lat, lng) {
    ...
    // Add accuracy circle (simulated ~10m accuracy)
    ...
    userMarker.bindPopup(isSimulating ? 'Walking simulation...' : 'Your current location');
    
    // Check if current location is over a Medium/High crime tile
    checkCurrentLocationTile(lat, lng);
  }
...
<div class="map-container">
  ...
  <div class="map-controls" class:mobile-open={showMobileMenu}>
    ...
    <button 
      class="control-button simulation-button" 
      class:active={isSimulating && !isPaused}
      class:paused={isPaused}
      on:click={toggleSimulation}
      title={!isSimulating ? "Start Walking" : isPaused ? "Resume Walking" : "Pause Walking"}
      disabled={pathCoordinates.length === 0}
    >
      {#if !isSimulating}
        🚶 Start Walking
      {:else if isPaused}
    ...
    <button 
      class="control-button overlay-mode-button" 
      class:off={overlayMode === 'off'}
      class:active-mode={overlayMode === 'active'}
      class:always={overlayMode === 'always'}
      on:click={cycleOverlayMode}
      title="Toggle Crime Overlay Mode"
    >
      {#if overlayMode === 'off'}
        🔕 Overlay: Off
      {:else if overlayMode === 'active'}
  ...
  <!-- Crime Warning Overlay -->
  {#if currentTileSeverity && (showUserLocation || isSimulating)}
    <div class="crime-warning-overlay" 
         class:low={currentTileSeverity === 'Low'}
         class:medium={currentTileSeverity === 'Medium'} 
         class:high={currentTileSeverity === 'High'}
         class:nodata={currentTileSeverity === 'NoData'}>
      <div class="warning-content">
        <div class="warning-icon">
          {#if currentTileSeverity === 'High'}
            ⚠️
          {:else if currentTileSeverity === 'Medium'}
            ⚡
          {:else if currentTileSeverity === 'Low'}
            ✓
          {:else}
            ❓
          {/if}
        ...
        <div class="warning-text">
          <h3>
            {#if currentTileSeverity === 'NoData'}
              No Crime Data
            {:else}
              {currentTileSeverity} Crime Area
            {/if}
          </h3>
          <p>
            {#if currentTileData}
              Crimes reported: {currentTileData.crime_count}
              <br>
              Crime Score: {currentTileData.weighted_crime_decayed_sum.toFixed(0)}
            {:else if currentTileSeverity === 'NoData'}
...
<style>
  ...
  .control-button.simulation-button.active {
    background: #9C27B0;
    border-color: #9C27B0;
  }
  
  .control-button.simulation-button.active:hover {
    background: #7B1FA2;
    border-color: #7B1FA2;
  }
  
  .control-button.simulation-button.paused {
    background: #FF9800;
    border-color: #FF9800;
    color: white;
  }
  
  .control-button.simulation-button.paused:hover {
    background: #F57C00;
    border-color: #F57C00;
  }
  
  .control-button.overlay-mode-button.off {
    background: #f5f5f5;
    border-color: #ddd;
    color: #666;
  }
  
  .control-button.overlay-mode-button.active-mode {
    background: #FF9800;
    border-color: #FF9800;
    color: white;
  }
  
  .control-button.overlay-mode-button.active-mode:hover {
    background: #F57C00;
    border-color: #F57C00;
  }
  
  .control-button.overlay-mode-button.always {
    background: #E91E63;
    border-color: #E91E63;
    color: white;
  }
  
  .control-button.overlay-mode-button.always:hover {
    background: #C2185B;
    border-color: #C2185B;
  }
  ...
  .crime-warning-overlay.medium {
    background: rgba(239, 108, 0, 0.95);
    border-top-color: #ef6c00;
  }
  
  .crime-warning-overlay.high {
    background: rgba(198, 40, 40, 0.95);
    border-top-color: #c62828;
  }
  
  .crime-warning-overlay.low {
    background: rgba(46, 125, 50, 0.95);
    border-top-color: #2e7d32;
  }
  
  .crime-warning-overlay.nodata {
